...
select train_no, departure_time from TRAIN_STATION@nabil where destinaton_station = st2;

cursor sts_tbl2(tn2 TRAIN_STATION.train_no%TYPE) is
select total_avail, ac_avail, g_avail from train_status@nabil where train_no = tn2;
...
select user_id, status, no_of_passengers, train_no from TICKET@nabil where id = t_id;

cursor ps_tbl2(tk_id TICKET.id%TYPE, u_id TICKET.user_id%TYPE) is 
select seat_number from passenger@nabil where ticket_id = tk_id and user_id = u_id;
